...
function validateAssignment(
  ...
  for (const [roleId, config] of Object.entries(ROLE_REGISTRY)) {
    ...
    for (const level of config.levels) {
      if (!roleData[level]) {
        logger.error({ roleId, level }, "Missing role level in LLM assignment");
        return null;
      }
...
${jsonExample}`;
  ...
  try {
    const sessionId = "fabrica-model-selection";
    ...
    logger.debug({ assignment }, "LLM returned model assignment");

    // Validate and backfill
    const validated = validateAssignment(assignment, availableModels[0].model);
    if (!validated) {
      logger.error({ assignment }, "Invalid assignment structure from LLM");
      throw new Error(
    ...
    logger.error({ err }, "LLM model selection failed");
    return null;
  }
}
