...
import { withTickMutex } from "./index.js";
...
export async function runHeartbeatSweep(opts: SweepOptions): Promise<HeartbeatSweepResult | null> {
  const logger = opts.logger ?? noopLogger;
  const agents = resolveAgents(opts.runtime, opts.workspaceDir, opts.agentId);

  const mutexResult = await withTickMutex(async () => {
  ...
  if (mutexResult === "busy") {
    logger.warn("[cli-sweep] Heartbeat tick running, skipping sweep");
    return null;
  }
  return mutexResult;
}

export async function runTriageSweep(opts: SweepOptions): Promise<HeartbeatSweepResult | null> {
  const logger = opts.logger ?? noopLogger;
  const agents = resolveAgents(opts.runtime, opts.workspaceDir, opts.agentId);
...
export async function runHealthSweep(opts: SweepOptions): Promise<HealthSweepResult | null> {
  const logger = opts.logger ?? noopLogger;
  const agents = resolveAgents(opts.runtime, opts.workspaceDir, opts.agentId);
