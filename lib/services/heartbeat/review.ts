      ...
      if (status.mergeable === false) {
        ...
        if (conflictTransition) {
          ...
          if (targetState) {
            ...
            // Track conflict in issueRuntime so work-finish can detect it reliably
            // even if audit.log has rotated by the time the developer calls work_finish
            {
              const projectsData = await readProjects(workspaceDir).catch(() => null);
              const proj = projectsData ? getProject(projectsData, projectName) : null;
              if (proj) {
                await updateIssueRuntime(workspaceDir, proj.slug, issue.iid, {
                  lastConflictDetectedAt: new Date().toISOString(),
                }).catch(() => {});
              }
            }
