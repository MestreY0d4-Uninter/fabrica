...
describe("E2E pipeline", () => {
  ...
  describe("dispatchTask", () => {
    ...
    it("should respawn when the slot still carries a legacy numeric session key", async () => {
      // Legacy numeric session keys no longer match the canonical deterministic
      // slot name, so dispatch should delete the stale session and spawn anew.
      h = await createTestHarness({
        workers: {
          developer: {
            level: "medior",
    ...
    it("should reuse an existing canonical session when the deterministic key matches", async () => {
      const canonicalSessionKey = `agent:test-agent:subagent:test-project-developer-medior-${slotName("test-project", "developer", "medior", 0).toLowerCase()}`;
      h = await createTestHarness({
        workers: {
          developer: {
            level: "medior",
      ...
      h.provider.seedIssue({ iid: 43, title: "Quick fix canonical", labels: ["To Do"] });
      ...
      const result = await dispatchTask({
        ...
        project: h.project,
        issueId: 43,
        issueTitle: "Quick fix canonical",
        issueDescription: "",
    ...
    it("should omit issue comments during feedback cycles and rely on PR feedback", async () => {
      h.provider.comments.set(42, [
        { id: 1001, author: "owner", body: "Historical issue discussion that should not drive review fixes", created_at: "2026-01-01T00:00:00Z" },
      ]);
      h.provider.setPrStatus(42, { state: "changes_requested", url: "https://example.com/pr/42" });
      h.provider.getPrReviewComments = async () => [
        { id: 1, author: "reviewer", body: "Please fix the PR body", state: "changes_requested" },
      ];
      ...
      const taskMsg = h.commands.taskMessages()[0];
      assert.ok(taskMsg.includes("## PR Review Feedback"), "Should include PR feedback section");
      assert.ok(!taskMsg.includes("Historical issue discussion"), "Should not replay issue comments during feedback cycle");
    });
  });
  ...
  describe("executeCompletion — reviewer", () => {
    ...
    it("reviewer:approve should transition Reviewing → To Test without merging directly", async () => {
  ...
  describe("executeCompletion — tester:pass", () => {
    ...
    it("refuses to close the issue when the canonical PR is still open", async () => {
      ...
      await assert.rejects(
        ...
        /Refusing to close issue #31 while canonical PR #310 is still open/,
      );
  ...
  describe("reviewPass", () => {
    ...
    it("should transition To Review → To Test when PR is approved (review:human)", async () => {
      ...
      assert.strictEqual(gitCmds.length, 0, "Should not run git pull when no merge action is configured");
    });
    ...
    it("should transition To Review → To Test when no merge action is configured, even if mergePr would fail", async () => {
      ...
      assert.ok(issue.labels.includes("To Test"), `Labels: ${issue.labels}`);
      assert.ok(!issue.labels.includes("To Review"), "Should not have To Review");
      assert.ok(!issue.labels.includes("To Improve"), "Should NOT have To Improve");
  ...
  describe("projectTick — reviewPolicy gating", () => {
    function workflowWithPolicy(policy: ReviewPolicy, testPolicy = DEFAULT_WORKFLOW.testPolicy): WorkflowConfig {
      return { ...DEFAULT_WORKFLOW, reviewPolicy: policy, testPolicy };
    }
    ...
    it("reviewPolicy: human should skip reviewer dispatch", async () => {
      ...
      const result = await projectTick({
        ...
        projectSlug: h.project.slug,
        targetRole: "reviewer",
        workflow: workflowWithPolicy(ReviewPolicy.HUMAN, TestPolicy.AGENT),
        provider: h.provider,
        runCommand: h.runCommand,
      });
    ...
    it("reviewPolicy: agent should dispatch reviewer", async () => {
      ...
      data.projects[h.project.slug]!.issueRuntime = {
        "81": {
          currentPrNumber: 810,
          currentPrState: "open",
          currentPrUrl: "https://example.com/pr/810",
          currentPrIssueTarget: 81,
        },
      };
    ...
    it("reviewPolicy: human should still allow developer dispatch while keeping reviewer blocked", async () => {
      h = await createTestHarness();
      h.provider.seedIssue({ iid: 84, title: "Dev task", labels: ["To Do"] });
      h.provider.seedIssue({ iid: 85, title: "Test task", labels: ["To Test"] });
      h.provider.setPrStatus(85, {
        number: 850,
        state: "open",
        url: "https://example.com/pr/850",
        currentIssueMatch: true,
      });
      const data = await h.readProjects();
      data.projects[h.project.slug]!.issueRuntime = {
        "85": {
          currentPrNumber: 850,
          currentPrState: "open",
          currentPrUrl: "https://example.com/pr/850",
          currentPrIssueTarget: 85,
        },
      };
      await h.writeProjects(data);
  ...
  describe("role:level labels", () => {
    ...
    it("dispatch should apply review routing from the workflow policy, not from developer level", async () => {
      h = await createTestHarness();
      ...
      assert.ok(issue.labels.includes("review:human"), `Should have review:human under the default policy, got: ${issue.labels}`);
    });
    ...
    it("projectTick should dispatch reviewer when review:agent label present", async () => {
      ...
      h.provider.setPrStatus(403, {
        number: 4030,
        state: "open",
        url: "https://example.com/pr/4030",
        currentIssueMatch: true,
      });
      const data = await h.readProjects();
      data.projects[h.project.slug]!.issueRuntime = {
        "403": {
          currentPrNumber: 4030,
          currentPrState: "open",
          currentPrUrl: "https://example.com/pr/4030",
          currentPrIssueTarget: 403,
        },
      };
      await h.writeProjects(data);
