 ...
 *
 * Run: corepack pnpm vitest run lib/services/bootstrap.e2e.test.ts
 */
...
describe("E2E bootstrap — extraSystemPrompt injection", () => {
  ...
  it("should inject scaffolded default instructions when no overrides exist", async () => {
    ...
    // Don't write any custom prompts — ensureDefaultFiles scaffolds packaged defaults.
    ...
    const prompts = h.commands.extraSystemPrompts();
    assert.strictEqual(prompts.length, 1, "Default packaged prompt should be injected");
    assert.ok(prompts[0].length > 0, "Default prompt content should not be empty");
  });
  ...
  it("should resolve tester instructions independently from developer", async () => {
    ...
    data.projects[h.project.slug]!.issueRuntime = {
      "4": {
        currentPrNumber: 400,
        currentPrState: "open",
        currentPrUrl: "https://example.com/pr/400",
        currentPrIssueTarget: 4,
      },
    };
    await h.writeProjects(data);
    h.project.issueRuntime = {
      "4": {
        currentPrNumber: 400,
        currentPrState: "open",
        currentPrUrl: "https://example.com/pr/400",
...
describe("E2E bootstrap — agent:bootstrap hook (AGENTS.md stripping)", () => {
  ...
  it("should strip AGENTS.md for Fabrica worker sessions", async () => {
    h = await createTestHarness({ projectName: "my-app" });
  ...
  it("should NOT strip AGENTS.md for non-Fabrica sessions", async () => {
