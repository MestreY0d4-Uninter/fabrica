...
function createCommandInterceptor(): {
  ...
  const gatewaySessions = new Map<string, { key: string; updatedAt: number; percentUsed: number; totalTokens: number; contextTokens: number }>();
  ...
  const handler = async (
    ...
    if (argv[0] === "openclaw" && argv[1] === "gateway" && argv[2] === "call") {
      ...
      if (paramsIdx !== -1 && argv[paramsIdx + 1]) {
        try {
          ...
          if (rpcMethod === "sessions.patch") {
            ...
            gatewaySessions.set(params.key, {
              key: params.key,
              updatedAt: Date.now(),
              percentUsed: 0,
              totalTokens: 100_000,
      ...
      if (rpcMethod === "sessions.delete" && paramsIdx !== -1 && argv[paramsIdx + 1]) {
        try {
          const params = JSON.parse(argv[paramsIdx + 1]);
          gatewaySessions.delete(params.key);
        } catch { /* ignore parse errors */ }
      }
    }
...
export type HarnessOptions = {
  ...
  baseBranch?: string;
  /**
   * Workflow config for provider-facing test semantics (default: DEFAULT_WORKFLOW).
   * This does not rewrite workspace workflow.yaml; dispatch/config resolution still
   * uses the packaged/default config unless the test writes workspace files itself.
   */
  workflow?: WorkflowConfig;
