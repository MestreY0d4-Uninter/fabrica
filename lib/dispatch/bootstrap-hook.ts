/**
 * bootstrap-hook.ts — Bootstrap support for Fabrica worker sessions.
 *
 * Provides:
 ...
 */
export function parseFabricaSessionKey(
  sessionKey: string,
...
/**
 * Register the agent:bootstrap hook for Fabrica worker sessions.
 *
...
export function registerBootstrapHook(api: OpenClawPluginApi, ctx: PluginContext): void {
  api.registerHook(
    ...
    {
      name: "fabrica-bootstrap-role-instructions",
      description:
        "Replaces orchestrator AGENTS.md with role-specific instructions for Fabrica workers",
    } as any,
  );
}
