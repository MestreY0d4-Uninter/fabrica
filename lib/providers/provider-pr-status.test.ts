...
describe("GitHubProvider.getPrStatus — closed PR handling", () => {
  ...
  it("ignores an open PR that was retargeted to another issue even if the branch still points to the old issue", async () => {
    const provider = new GitHubProvider({ repoPath: "/fake", runCommand: mockRunCommand });
  ...
  it("chooses the most recently merged PR when multiple merged PRs exist", async () => {
    const provider = new GitHubProvider({ repoPath: "/fake", runCommand: mockRunCommand });

    const olderMergedUrl = "https://github.com/owner/repo/pull/5";
    const newerMergedUrl = "https://github.com/owner/repo/pull/9";
        ...
        return [
          {
            title: "older merged",
            body: "",
            headRefName: "feature/5-older",
            url: olderMergedUrl,
            reviewDecision: null,
            mergedAt: "2026-03-12T22:28:17Z",
          },
          {
            title: "newer merged",
            body: "",
            headRefName: "feature/9-newer",
            url: newerMergedUrl,
            reviewDecision: null,
            mergedAt: "2026-03-13T00:04:06Z",
          },
        ];
      }
      return [];
    ...
    assert.strictEqual(status.state, PrState.MERGED);
    assert.strictEqual(status.url, newerMergedUrl);
    assert.strictEqual(status.sourceBranch, "feature/9-newer");
  });
