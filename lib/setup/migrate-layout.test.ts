...
describe("migrateWorkspaceLayout — very old layout → fabrica/", () => {
  it("should move projects/projects.json to fabrica/projects.json", async () => {
    ...
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "projects.json")), "projects.json should be at fabrica/");
  ...
  it("should rename projects/config.yaml to fabrica/workflow.yaml", async () => {
    ...
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "workflow.yaml")), "workflow.yaml should be at fabrica/");
    assert.ok(!await fileExists(path.join(projDir, "config.yaml")), "old config.yaml should be removed");
    const content = await fs.readFile(path.join(tmpDir, DATA_DIR, "workflow.yaml"), "utf-8");
  ...
  it("should move roles/default/* to fabrica/prompts/ with renames", async () => {
    ...
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "prompts", "developer.md")), "dev.md should become developer.md");
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "prompts", "tester.md")), "qa.md should become tester.md");
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "prompts", "architect.md")), "architect.md should stay");

    const devContent = await fs.readFile(path.join(tmpDir, DATA_DIR, "prompts", "developer.md"), "utf-8");
  ...
  it("should move roles/<project>/* to fabrica/projects/<project>/prompts/ with renames", async () => {
    ...
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "projects", "my-app", "prompts", "developer.md")), "dev.md should become prompts/developer.md");
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "projects", "my-app", "prompts", "tester.md")), "qa.md should become prompts/tester.md");
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "projects", "my-app", "prompts", "architect.md")), "architect.md should be in prompts/");

    const content = await fs.readFile(path.join(tmpDir, DATA_DIR, "projects", "my-app", "prompts", "developer.md"), "utf-8");
  ...
  it("should rename projects/<project>/config.yaml to fabrica/projects/<project>/workflow.yaml", async () => {
    ...
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "projects", "my-app", "workflow.yaml")), "workflow.yaml should exist");
  ...
  it("should move log/ to fabrica/log/", async () => {
    ...
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "log", "audit.log")), "audit.log should be in fabrica/log/");
...
describe("migrateWorkspaceLayout — intermediate layout → fabrica/", () => {
  it("should move projects.json from root to fabrica/", async () => {
    ...
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "projects.json")), "projects.json should be in fabrica/");
  ...
  it("should move workflow.yaml from root to fabrica/", async () => {
    ...
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "workflow.yaml")), "workflow.yaml should be in fabrica/");
  ...
  it("should move prompts/ from root to fabrica/prompts/", async () => {
    ...
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "prompts", "developer.md")), "developer.md should be in fabrica/prompts/");
  ...
  it("should move project .md files into prompts/ subdir", async () => {
    ...
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "projects", "my-app", "prompts", "developer.md")), "developer.md should be in prompts/ subdir");
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "projects", "my-app", "workflow.yaml")), "workflow.yaml should stay at project root");
...
describe("migrateWorkspaceLayout — no-op cases", () => {
  it("migrates a legacy devclaw/ data dir into fabrica/", async () => {
    ...
    await fs.writeFile(path.join(legacyDir, "log", "audit.log"), '{"ts":"2024-01-01"}');

    await migrateWorkspaceLayout(tmpDir);

    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "projects.json")), "projects.json should be migrated to fabrica/");
    assert.ok(await fileExists(path.join(tmpDir, DATA_DIR, "log", "audit.log")), "log should be migrated to fabrica/");
    assert.ok(!await fileExists(legacyDir), "legacy devclaw/ directory should be removed");
