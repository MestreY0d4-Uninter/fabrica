...
/**
 * Write Fabrica plugin config to openclaw.json plugins section.
 *
 * Configures:
 * - Tool restrictions (deny sessions_spawn, sessions_send) for Fabrica-created agents
...
export async function writePluginConfig(
  ...
  if (projectExecution) {
    (config as any).plugins.entries.fabrica.config.projectExecution = projectExecution;
  }
...
function ensurePluginStructure(config: Record<string, unknown>): void {
  ...
  if (!entries.fabrica) entries.fabrica = {};
  const fabrica = entries.fabrica as Record<string, unknown>;
  if (!fabrica.config) fabrica.config = {};
  fabrica.enabled = true;
}

/**
 * Ensure "fabrica" is in plugins.allow so OpenClaw trusts the plugin
