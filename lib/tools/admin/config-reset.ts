...
export function createConfigResetTool(_ctx: PluginContext) {
  return (toolCtx: ToolContext) => ({
    ...
    label: "Config Reset",
    description:
      "Reset Fabrica config files to defaults. Creates .bak backups before overwriting. " +
    ...
    parameters: {
      ...
      properties: {
        ...
        target: {
          ...
          description:
            "What to reset. 'workflow' = workflow.yaml, 'prompts' = fabrica/prompts/*.md, 'all' = both. Default: 'all'.",
        },
      },
      required: ["channelId"],
    },
    execute: async (params: Record<string, unknown>) => {
      ...
      if (target === "workflow" || target === "all") {
        ...
        resetFiles.push("fabrica/workflow.yaml");
      }
      ...
      if (target === "prompts" || target === "all") {
        ...
        for (const role of getAllRoleIds()) {
          ...
          resetFiles.push(`fabrica/prompts/${role}.md`);
        }
      }
