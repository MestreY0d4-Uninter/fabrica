...
import { readProjects, writeProjects, buildRouteRef, routeMatchesChannel } from "../../projects/index.js";
...
export function createChannelUnlinkTool(_ctx: PluginContext) {
  return (toolCtx: ToolContext) => ({
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      // Find the channel
      const idx = target.channels.findIndex((ch) => routeMatchesChannel(targetRoute, ch));
      if (idx === -1) {
        throw new Error(
          `Channel route ${channelId}${messageThreadId ? `:${messageThreadId}` : ""} not found in project "${target.name}".`,
        );
      }
