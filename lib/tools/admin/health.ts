...
export function createHealthTool(ctx: PluginContext) {
  return (toolCtx: ToolContext) => ({
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      if (slugOrChannelId) {
        const route = resolveRoute(toolCtx, slugOrChannelId);
        const exact = getProjectByRoute(data, route);
        const slug = exact?.slug ?? (data.projects[slugOrChannelId] ? slugOrChannelId : undefined);
        slugs = slug ? [slug] : [];
      }
