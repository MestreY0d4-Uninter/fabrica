...
export function createTaskCreateTool(ctx: PluginContext) {
  return (toolCtx: ToolContext) => ({
    name: "task_create",
    label: "Task Create",
    description: "Create a new task (issue) in the project's issue tracker. Issues are created in the project's initial workflow state.",
    parameters: {
      type: "object",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      applyNotifyLabel(
        ...
        issue.iid,
        project,
        primaryChannel?.channelId ?? route.channelId,
        undefined,
