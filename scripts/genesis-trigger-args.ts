...
export function parseGenesisArgs(args: string[]): GenesisArgs & { error?: string } {
  ...
  if (isNaN(parseInt(rawChannelId, 10))) {
    return { rawIdea, channelId: "", dryRun, error: "--channel-id must be a numeric value" };
  }
